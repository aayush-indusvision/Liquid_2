...
os.system("title LPK_Cam_4_Open_Pouch_2")
...
def initialize_model():
    ...
    model = YOLO(r"models\22_01_25_LPK_Open_Pouch_2.engine",task='segment')
...
def rejected_and_sent_to_dashboard(frame, plc_reject, defect_id,product,rca_id):
    ...
    dashboard.send_image(encoded_image, defect=defect_id,product=product,rca=rca_id,original_frame=frame)
    return
...
def detect_defects(model, frame,size):
    ...
    results = model.predict(frame, conf=0.85, verbose=False,classes=[0])
...
def main_loop(size, product, stop_event, frame_queue):
    ...
    plc = None
    if ((isinstance(ping('192.168.21.14'), (int, float))) and (isinstance(ping('192.168.21.10'), (int, float)))):
    # if ((isinstance(ping('192.168.21.14'), (int, float)))): # USE IN-CASE PLC IS NOT TO BE USED
    ...
    try:
        while not stop_event.is_set():
            ...
            try:
                # Check the stop event regularly
                    if ((not isinstance(ping('192.168.21.14'), (int, float))) or (not isinstance(ping('192.168.21.10'), (int, float)))):
                    # if ((not isinstance(ping('192.168.21.14'), (int, float)))): # USE IN-CASE PLC IS NOT TO BE USED
                        no_frame_count+=1
                        ...
                        cv2.imshow('LPK_Open_Pouch_2', image)
                        cv2.waitKey(1)
                    ...
                    cv2.imshow('LPK_Open_Pouch_2', resized_frame)
                    cv2.waitKey(1)  
                    ...
                    if send_trigger:
                        ...
                        if defect_count%3==0:
                            rca=1
                        rejected_and_sent_to_dashboard(frame=annotated_frame,plc_reject=plc,defect_id=81,product=product,rca_id=rca)
                        print("----------------Defect-----------------------")
                    else:
                        defect_count=0
                        rca=0
        ...
        if os.path.exists(r"C:\Users\pc\Desktop\Open_pouch_model-20240830T104935Z-001\Open_Pouch\Open_Pouch_2\script_2.lock"):
            os.remove(r"C:\Users\pc\Desktop\Open_pouch_model-20240830T104935Z-001\Open_Pouch\Open_Pouch_2\script_2.lock")
        if status_flag==0:
            cv2.imshow('LPK_Open_Pouch_2', image)
            cv2.waitKey(0)
        else:
...
class FunctionSelector(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("LPK Open Pouch 2")
